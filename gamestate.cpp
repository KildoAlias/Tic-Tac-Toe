...
std::string GameState::toString(int pPlayer) const
{
	...
	for (int c = 0; c < 4; ++c)
		ss << cell_text[at(0, c)];
	ss << board_right << " 3   Last move: " + mLastMove.toString() + (is_winner ? " (WOHO! I WON!)\n" : "\n");
	ss << "  4 " << board_left;
	for (int c = 0; c < 4; ++c)
		ss << cell_text[at(1, c)];
	ss << board_right << " 7   Next player: " + cell_text[mNextPlayer] + (is_my_turn ? " (My turn)\n" : " (Opponents turn)\n");
	ss << "  8 " << board_left;
	for (int c = 0; c < 4; ++c)
		ss << cell_text[at(2, c)];
	ss << board_right << " 11  X pieces:   " << X_pieces << "\n";
	ss << " 12 " << board_left;
	for (int c = 0; c < 4; ++c)
		ss << cell_text[at(3, c)];
	ss << board_right << " 15  O pieces: " << O_pieces << "\n";
	ss << board_bottom;
